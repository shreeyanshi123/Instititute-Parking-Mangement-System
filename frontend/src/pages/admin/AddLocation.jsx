...
const AddLocation = () => {
  ...
  const [uploadedImageUrl, setUploadedImageUrl] = useState('');
  const [isLoadingState, setImageLoadingState] = useState(false);
  const [slots, setSlots] = useState({ twoWheeler: "", fourWheeler: "", bus: "" });

  const handleChange = (e, type) => {
    const value = e.target.value;
    setSlots((prevSlots) => ({
      ...prevSlots,
      [type]: value === "" ? "" : Math.max(0, Number(value)), // Prevents negative numbers
    }));
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden add-location p-6">
      ...
      <button
        onClick={() => navigate("/admin/home")}
        className="absolute top-5 left-5 px-4 py-2 bg-gray-800 text-white rounded-lg shadow-md hover:bg-gray-800 transition-all"
      >
        🔙 Back to Home
      </button>

      {/* Form Container */}
      <div className="add-location-card relative z-10 w-full  max-w-lg bg-opacity-10 backdrop-blur-md shadow-xl rounded-2xl mb-8 p-8 border border-white/20">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <LocationImageUpload imageFile={imageFile} setImageFile={setImageFile} uploadedImageUrl={uploadedImageUrl} setUploadedImageUrl={setUploadedImageUrl} setImageLoadingState={setImageLoadingState} isLoadingState={isLoadingState}  />
            ...
            <label className="block text-white text-lg font-semibold mt-3"> Location Name</label>
            <input
              type="text"
              placeholder="Enter Location Name"
              value={locationName}
